...
class DefaultCostFunction:
    ...
    def calculate_costs(self, worker, service):
        ...
        cpu_costs, vram_costs, swap_costs, image_download_costs = self.calculate_partial_costs(cpu_usage_of_worker,
                                                                                               vram_usage_of_worker,
                                                                                               swap_usage_of_worker,
                                                                                               size_of_docker_image,
                                                                                               bandwidth_in_percent
                                                                                               )
        overall_costs = self.calculate_overall_costs(cpu_costs, vram_costs, swap_costs, image_download_costs)
    ...
    def calculate_overall_costs(self, cpu_costs, vram_costs, swap_costs, image_download_costs):
    ...
    def calculate_partial_costs(self, cpu_usage_of_worker, vram_usage_of_worker, swap_usage_of_worker,
    ...
    def get_usage_of_worker(self, worker, service):
        # Get logger for local logging
        llogger = local_logger.LocalLogger()
        # Log function call
        llogger.debug("Call: %s", sys._getframe().f_code.co_name)
