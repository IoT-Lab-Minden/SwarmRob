...
class Argument(Enum):
    ...
    """
    VERBOSE = '-v', "--verbose", "verbose", "store_true", None, False, 'Enable verbose logging mode'
    REPOSITORY = '-r', "--repository", "repository", "store", str, True, ''
    ADVERTISE_ADDRESS = '-a', '--advertise_address', "advertise_address", "store", str, False,\
                        'Advertise address used to access the daemon'
    INTERFACE = '-i', '--interface', "interface", "store", str, False, 'Interface to use for and connect to the daemon'
    UUID_OPTIONAL = '-u', '--uuid', "uuid", "store", str, False, 'UUID of the object'
    UUID = '-u', '--uuid', "uuid", "store", str, True, 'UUID of the object'
    COMPOSE_FILE = '-c', '--compose_file', "compose_file", "store", str, True, 'Path to the edf file'
    LOG_IDENTIFIER = '-l', '--log_identifier', "log_identifier", "store", str, False, 'Identifier for a log file'
    LOG_FOLDER = '-f', '--log_folder', "log_folder", "store", str, False, 'Path to save evaluation logs in'
    WORKER_UUID = '-w', '--worker_uuid', "worker_uuid", "store", str, True, 'UUID of the worker'
    SWARM_UUID = '-s', '--swarm_uuid', "swarm_uuid", "store", str, True, 'UUID of the swarm'
    DAEMONIZE = '-n', '--no-daemon', "daemonize", "store_false", None, False, 'Run swarmrob daemon in background'

    def __new__(cls, short_name, long_name, destination, action, data_type, required, help_msg):
        """
...
class CMDParser:
    ...
    def parse_arguments(self):
        ...
        """
        params = self._arg_parser.parse_known_args()
        return params[0]
